...
The whole script has been split in five scripts for practicale reasons. Each one
...
%%%
\itbox Writing the tidy data set in \textit{.csv} files and this \textit{codeBook.pdf}.
The current document use all \MyHref{myblue}\href{http://latex-project.org/intro.html}
{\LaTeX} high quality text processor facilities.
